...
```
STOPPING POINT, work on turning sections of code into functions.

start with doc matching function I started but got stuck

check warning messages on rayleigh function --> when widths overlap and it tries to deal with NA (not sure how to fix but can ignore for now) [quieted, but should ideally figure out how to fix error without just quieting it]

function to set up file structure??
...
1. Figure out how to save data file for doc function 
2. Add run type into metadata table 
3. Ensure sampleQ/manual have consistent capitalization


#Work Flow 
1. Finish converting functions written in code to functions in package
2. Make sure updated functions are correct in the code (mostly done minus indicies functions)
3. Make sure code runs 
