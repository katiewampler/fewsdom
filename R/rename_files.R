...
#' @param zip_files Will create a zip file in the file directory of all the raw, nonrenamed files from the Aqualog as a backup
#' @export
...
clean_files <- function(prjpath, meta_file, meta_sheet, zip_files=T){
  ...
  stopifnot(sum(stringr::str_detect(list.files(prjpath), "SEM")) +
